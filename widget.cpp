...
Widget::Widget(QWidget *parent)
    : QWidget(parent), m_drag(DragMove_None)
{
...
void Widget::checkMouseMoveRegion()
{
    bool activeFlag;
    region(cursor().pos(), activeFlag);
}

bool Widget::checkLButtonPressRegion()
{
    bool activeFlag;
    region(cursor().pos(), activeFlag);
...
void Widget::checkSize(void *arg)
{
...
void Widget::checkDragMove(void *arg)
{
...
void Widget::resizeEvent(QResizeEvent *event)
{
    ...
    if(height() != qApp->desktop()->availableGeometry().height())
        m_normalSize = size();
    resizeBackground(width(), height(), 5, 5, "#80C7ED");//set your like
}
